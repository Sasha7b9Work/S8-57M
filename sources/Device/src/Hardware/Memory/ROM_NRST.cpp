...
{
    PacketNRST *last = LastPacket();                            // Находим последний сохранённый в секторе пакет

    if(last == nullptr)                                     // Если пакетов ещё нету
    {
        last = reinterpret_cast<PacketNRST *>(sector.address);  // то он будет в самом начале сектора
