...
int DisplayOsci::HiPart::DrawMainParameters(int _x, int _y)
{
    ...
    String("р : %s", TBase::ToString(S_TIME_BASE)).Draw(x, y0, Color::FILL);
    ...
    {
        int d = 8;
        WriteStringAndNumber("НАКОПЛ", x, y0 - 6 + d, 0, ENumAccum::ToString(S_DISP_ENUM_ACCUM));
        WriteStringAndNumber("УСРЕДН", x, y0 + 6 + d, S_OSCI_NUM_AVERAGE);
        WriteStringAndNumber("СГЛАЖ", x, y0 + 18 + d, S_DISP_NUM_SMOOTH);
    }
...
void DisplayOsci::HiPart::WriteTextVoltage(Ch::E ch, int x, int y)
{
    ...
    String((ch == ChA) ? "1к" : "2к").Draw(x + 1, y, colorDraw);
    String(":").Draw(x + 18, y);
