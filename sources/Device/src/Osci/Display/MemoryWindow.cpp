...
static void DrawScreenArea()
{
    float relBegin = (float)DisplayOsci::ShiftInMemory::Get() / ENumPointsFPGA::PointsInChannel();          // Относительное начало экранной области относительно всей памяти
    float relWidth = (float)(Grid::Width() + 1) / ENumPointsFPGA::PointsInChannel();        // Относительное (относительно всей шириный окна) ширина видимой области
